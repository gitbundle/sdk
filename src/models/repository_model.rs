/*
 * GitBundle API
 *
 * GitBundle API documentation.
 *
 * The version of the OpenAPI document: 3.0.0
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct RepositoryModel {
    #[serde(rename = "created")]
    pub created: i64,
    #[serde(rename = "created_by")]
    pub created_by: i64,
    #[serde(rename = "default_branch")]
    pub default_branch: String,
    #[serde(rename = "deleted", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub deleted: Option<Option<i64>>,
    #[serde(rename = "description")]
    pub description: String,
    #[serde(rename = "display_name")]
    pub display_name: String,
    #[serde(rename = "fork_id", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub fork_id: Option<Option<i64>>,
    #[serde(rename = "git_ssh_url", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub git_ssh_url: Option<Option<String>>,
    #[serde(rename = "git_uid")]
    pub git_uid: String,
    #[serde(rename = "git_url", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub git_url: Option<Option<String>>,
    #[serde(rename = "group_id")]
    pub group_id: i64,
    #[serde(rename = "id")]
    pub id: i64,
    #[serde(rename = "importing")]
    pub importing: bool,
    #[serde(rename = "is_empty")]
    pub is_empty: bool,
    #[serde(rename = "is_public")]
    pub is_public: bool,
    #[serde(rename = "name")]
    pub name: String,
    #[serde(rename = "num_closed_pulls")]
    pub num_closed_pulls: i64,
    #[serde(rename = "num_forks")]
    pub num_forks: i64,
    #[serde(rename = "num_merged_pulls")]
    pub num_merged_pulls: i64,
    #[serde(rename = "num_open_pulls")]
    pub num_open_pulls: i64,
    #[serde(rename = "num_pulls")]
    pub num_pulls: i64,
    #[serde(rename = "num_stars")]
    pub num_stars: i64,
    #[serde(rename = "path", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub path: Option<Option<String>>,
    #[serde(rename = "pullreq_seq")]
    pub pullreq_seq: i64,
    #[serde(rename = "size")]
    pub size: i64,
    #[serde(rename = "size_updated", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub size_updated: Option<Option<i64>>,
    #[serde(rename = "updated")]
    pub updated: i64,
    #[serde(rename = "version")]
    pub version: i64,
}

impl RepositoryModel {
    pub fn new(created: i64, created_by: i64, default_branch: String, description: String, display_name: String, git_uid: String, group_id: i64, id: i64, importing: bool, is_empty: bool, is_public: bool, name: String, num_closed_pulls: i64, num_forks: i64, num_merged_pulls: i64, num_open_pulls: i64, num_pulls: i64, num_stars: i64, pullreq_seq: i64, size: i64, updated: i64, version: i64) -> RepositoryModel {
        RepositoryModel {
            created,
            created_by,
            default_branch,
            deleted: None,
            description,
            display_name,
            fork_id: None,
            git_ssh_url: None,
            git_uid,
            git_url: None,
            group_id,
            id,
            importing,
            is_empty,
            is_public,
            name,
            num_closed_pulls,
            num_forks,
            num_merged_pulls,
            num_open_pulls,
            num_pulls,
            num_stars,
            path: None,
            pullreq_seq,
            size,
            size_updated: None,
            updated,
            version,
        }
    }
}

