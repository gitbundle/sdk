/*
GitBundle API

GitBundle API documentation.

API version: 3.3.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package gitbundlesdk

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the StageModel type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &StageModel{}

// StageModel struct for StageModel
type StageModel struct {
	ActionId      int64             `json:"action_id"`
	Arch          string            `json:"arch"`
	Created       int64             `json:"created"`
	Errignore     bool              `json:"errignore"`
	Error         string            `json:"error"`
	ExitCode      int64             `json:"exit_code"`
	Id            int64             `json:"id"`
	IsConcurrent  bool              `json:"is_concurrent"`
	IsMatrix      bool              `json:"is_matrix"`
	IsResolved    bool              `json:"is_resolved"`
	IsReusable    bool              `json:"is_reusable"`
	JobKey        string            `json:"job_key"`
	Kernel        string            `json:"kernel"`
	Kind          string            `json:"kind"`
	Labels        []string          `json:"labels"`
	Limit         int64             `json:"limit"`
	LimitRepo     int64             `json:"limit_repo"`
	Machine       string            `json:"machine"`
	Name          string            `json:"name"`
	Needs         []string          `json:"needs"`
	Number        int64             `json:"number"`
	OnFailure     bool              `json:"on_failure"`
	OnSuccess     bool              `json:"on_success"`
	Os            string            `json:"os"`
	Outputs       map[string]string `json:"outputs,omitempty"`
	ParentGroupId int64             `json:"parent_group_id"`
	ParentId      NullableInt64     `json:"parent_id,omitempty"`
	RepoId        int64             `json:"repo_id"`
	Started       NullableInt64     `json:"started,omitempty"`
	Status        CIStatus          `json:"status"`
	Stopped       NullableInt64     `json:"stopped,omitempty"`
	Type          string            `json:"type"`
	Updated       int64             `json:"updated"`
	Variant       string            `json:"variant"`
	Version       int64             `json:"version"`
	WorkflowId    int64             `json:"workflow_id"`
	YamlProvider  WorkflowProvider  `json:"yaml_provider"`
	YamlResolved  string            `json:"yaml_resolved"`
	YamlUses      NullableString    `json:"yaml_uses,omitempty"`
}

type _StageModel StageModel

// NewStageModel instantiates a new StageModel object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewStageModel(actionId int64, arch string, created int64, errignore bool, error_ string, exitCode int64, id int64, isConcurrent bool, isMatrix bool, isResolved bool, isReusable bool, jobKey string, kernel string, kind string, labels []string, limit int64, limitRepo int64, machine string, name string, needs []string, number int64, onFailure bool, onSuccess bool, os string, parentGroupId int64, repoId int64, status CIStatus, type_ string, updated int64, variant string, version int64, workflowId int64, yamlProvider WorkflowProvider, yamlResolved string) *StageModel {
	this := StageModel{}
	this.ActionId = actionId
	this.Arch = arch
	this.Created = created
	this.Errignore = errignore
	this.Error = error_
	this.ExitCode = exitCode
	this.Id = id
	this.IsConcurrent = isConcurrent
	this.IsMatrix = isMatrix
	this.IsResolved = isResolved
	this.IsReusable = isReusable
	this.JobKey = jobKey
	this.Kernel = kernel
	this.Kind = kind
	this.Labels = labels
	this.Limit = limit
	this.LimitRepo = limitRepo
	this.Machine = machine
	this.Name = name
	this.Needs = needs
	this.Number = number
	this.OnFailure = onFailure
	this.OnSuccess = onSuccess
	this.Os = os
	this.ParentGroupId = parentGroupId
	this.RepoId = repoId
	this.Status = status
	this.Type = type_
	this.Updated = updated
	this.Variant = variant
	this.Version = version
	this.WorkflowId = workflowId
	this.YamlProvider = yamlProvider
	this.YamlResolved = yamlResolved
	return &this
}

// NewStageModelWithDefaults instantiates a new StageModel object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewStageModelWithDefaults() *StageModel {
	this := StageModel{}
	return &this
}

// GetActionId returns the ActionId field value
func (o *StageModel) GetActionId() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.ActionId
}

// GetActionIdOk returns a tuple with the ActionId field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetActionIdOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ActionId, true
}

// SetActionId sets field value
func (o *StageModel) SetActionId(v int64) {
	o.ActionId = v
}

// GetArch returns the Arch field value
func (o *StageModel) GetArch() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Arch
}

// GetArchOk returns a tuple with the Arch field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetArchOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Arch, true
}

// SetArch sets field value
func (o *StageModel) SetArch(v string) {
	o.Arch = v
}

// GetCreated returns the Created field value
func (o *StageModel) GetCreated() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.Created
}

// GetCreatedOk returns a tuple with the Created field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetCreatedOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Created, true
}

// SetCreated sets field value
func (o *StageModel) SetCreated(v int64) {
	o.Created = v
}

// GetErrignore returns the Errignore field value
func (o *StageModel) GetErrignore() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.Errignore
}

// GetErrignoreOk returns a tuple with the Errignore field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetErrignoreOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Errignore, true
}

// SetErrignore sets field value
func (o *StageModel) SetErrignore(v bool) {
	o.Errignore = v
}

// GetError returns the Error field value
func (o *StageModel) GetError() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Error
}

// GetErrorOk returns a tuple with the Error field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetErrorOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Error, true
}

// SetError sets field value
func (o *StageModel) SetError(v string) {
	o.Error = v
}

// GetExitCode returns the ExitCode field value
func (o *StageModel) GetExitCode() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.ExitCode
}

// GetExitCodeOk returns a tuple with the ExitCode field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetExitCodeOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ExitCode, true
}

// SetExitCode sets field value
func (o *StageModel) SetExitCode(v int64) {
	o.ExitCode = v
}

// GetId returns the Id field value
func (o *StageModel) GetId() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetIdOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *StageModel) SetId(v int64) {
	o.Id = v
}

// GetIsConcurrent returns the IsConcurrent field value
func (o *StageModel) GetIsConcurrent() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.IsConcurrent
}

// GetIsConcurrentOk returns a tuple with the IsConcurrent field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetIsConcurrentOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.IsConcurrent, true
}

// SetIsConcurrent sets field value
func (o *StageModel) SetIsConcurrent(v bool) {
	o.IsConcurrent = v
}

// GetIsMatrix returns the IsMatrix field value
func (o *StageModel) GetIsMatrix() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.IsMatrix
}

// GetIsMatrixOk returns a tuple with the IsMatrix field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetIsMatrixOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.IsMatrix, true
}

// SetIsMatrix sets field value
func (o *StageModel) SetIsMatrix(v bool) {
	o.IsMatrix = v
}

// GetIsResolved returns the IsResolved field value
func (o *StageModel) GetIsResolved() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.IsResolved
}

// GetIsResolvedOk returns a tuple with the IsResolved field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetIsResolvedOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.IsResolved, true
}

// SetIsResolved sets field value
func (o *StageModel) SetIsResolved(v bool) {
	o.IsResolved = v
}

// GetIsReusable returns the IsReusable field value
func (o *StageModel) GetIsReusable() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.IsReusable
}

// GetIsReusableOk returns a tuple with the IsReusable field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetIsReusableOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.IsReusable, true
}

// SetIsReusable sets field value
func (o *StageModel) SetIsReusable(v bool) {
	o.IsReusable = v
}

// GetJobKey returns the JobKey field value
func (o *StageModel) GetJobKey() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.JobKey
}

// GetJobKeyOk returns a tuple with the JobKey field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetJobKeyOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.JobKey, true
}

// SetJobKey sets field value
func (o *StageModel) SetJobKey(v string) {
	o.JobKey = v
}

// GetKernel returns the Kernel field value
func (o *StageModel) GetKernel() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Kernel
}

// GetKernelOk returns a tuple with the Kernel field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetKernelOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Kernel, true
}

// SetKernel sets field value
func (o *StageModel) SetKernel(v string) {
	o.Kernel = v
}

// GetKind returns the Kind field value
func (o *StageModel) GetKind() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Kind
}

// GetKindOk returns a tuple with the Kind field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetKindOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Kind, true
}

// SetKind sets field value
func (o *StageModel) SetKind(v string) {
	o.Kind = v
}

// GetLabels returns the Labels field value
func (o *StageModel) GetLabels() []string {
	if o == nil {
		var ret []string
		return ret
	}

	return o.Labels
}

// GetLabelsOk returns a tuple with the Labels field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetLabelsOk() ([]string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Labels, true
}

// SetLabels sets field value
func (o *StageModel) SetLabels(v []string) {
	o.Labels = v
}

// GetLimit returns the Limit field value
func (o *StageModel) GetLimit() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.Limit
}

// GetLimitOk returns a tuple with the Limit field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetLimitOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Limit, true
}

// SetLimit sets field value
func (o *StageModel) SetLimit(v int64) {
	o.Limit = v
}

// GetLimitRepo returns the LimitRepo field value
func (o *StageModel) GetLimitRepo() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.LimitRepo
}

// GetLimitRepoOk returns a tuple with the LimitRepo field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetLimitRepoOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.LimitRepo, true
}

// SetLimitRepo sets field value
func (o *StageModel) SetLimitRepo(v int64) {
	o.LimitRepo = v
}

// GetMachine returns the Machine field value
func (o *StageModel) GetMachine() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Machine
}

// GetMachineOk returns a tuple with the Machine field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetMachineOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Machine, true
}

// SetMachine sets field value
func (o *StageModel) SetMachine(v string) {
	o.Machine = v
}

// GetName returns the Name field value
func (o *StageModel) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *StageModel) SetName(v string) {
	o.Name = v
}

// GetNeeds returns the Needs field value
func (o *StageModel) GetNeeds() []string {
	if o == nil {
		var ret []string
		return ret
	}

	return o.Needs
}

// GetNeedsOk returns a tuple with the Needs field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetNeedsOk() ([]string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Needs, true
}

// SetNeeds sets field value
func (o *StageModel) SetNeeds(v []string) {
	o.Needs = v
}

// GetNumber returns the Number field value
func (o *StageModel) GetNumber() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.Number
}

// GetNumberOk returns a tuple with the Number field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetNumberOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Number, true
}

// SetNumber sets field value
func (o *StageModel) SetNumber(v int64) {
	o.Number = v
}

// GetOnFailure returns the OnFailure field value
func (o *StageModel) GetOnFailure() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.OnFailure
}

// GetOnFailureOk returns a tuple with the OnFailure field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetOnFailureOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OnFailure, true
}

// SetOnFailure sets field value
func (o *StageModel) SetOnFailure(v bool) {
	o.OnFailure = v
}

// GetOnSuccess returns the OnSuccess field value
func (o *StageModel) GetOnSuccess() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.OnSuccess
}

// GetOnSuccessOk returns a tuple with the OnSuccess field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetOnSuccessOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OnSuccess, true
}

// SetOnSuccess sets field value
func (o *StageModel) SetOnSuccess(v bool) {
	o.OnSuccess = v
}

// GetOs returns the Os field value
func (o *StageModel) GetOs() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Os
}

// GetOsOk returns a tuple with the Os field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetOsOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Os, true
}

// SetOs sets field value
func (o *StageModel) SetOs(v string) {
	o.Os = v
}

// GetOutputs returns the Outputs field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *StageModel) GetOutputs() map[string]string {
	if o == nil {
		var ret map[string]string
		return ret
	}
	return o.Outputs
}

// GetOutputsOk returns a tuple with the Outputs field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *StageModel) GetOutputsOk() (map[string]string, bool) {
	if o == nil || IsNil(o.Outputs) {
		return map[string]string{}, false
	}
	return o.Outputs, true
}

// HasOutputs returns a boolean if a field has been set.
func (o *StageModel) HasOutputs() bool {
	if o != nil && !IsNil(o.Outputs) {
		return true
	}

	return false
}

// SetOutputs gets a reference to the given map[string]string and assigns it to the Outputs field.
func (o *StageModel) SetOutputs(v map[string]string) {
	o.Outputs = v
}

// GetParentGroupId returns the ParentGroupId field value
func (o *StageModel) GetParentGroupId() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.ParentGroupId
}

// GetParentGroupIdOk returns a tuple with the ParentGroupId field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetParentGroupIdOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ParentGroupId, true
}

// SetParentGroupId sets field value
func (o *StageModel) SetParentGroupId(v int64) {
	o.ParentGroupId = v
}

// GetParentId returns the ParentId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *StageModel) GetParentId() int64 {
	if o == nil || IsNil(o.ParentId.Get()) {
		var ret int64
		return ret
	}
	return *o.ParentId.Get()
}

// GetParentIdOk returns a tuple with the ParentId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *StageModel) GetParentIdOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return o.ParentId.Get(), o.ParentId.IsSet()
}

// HasParentId returns a boolean if a field has been set.
func (o *StageModel) HasParentId() bool {
	if o != nil && o.ParentId.IsSet() {
		return true
	}

	return false
}

// SetParentId gets a reference to the given NullableInt64 and assigns it to the ParentId field.
func (o *StageModel) SetParentId(v int64) {
	o.ParentId.Set(&v)
}

// SetParentIdNil sets the value for ParentId to be an explicit nil
func (o *StageModel) SetParentIdNil() {
	o.ParentId.Set(nil)
}

// UnsetParentId ensures that no value is present for ParentId, not even an explicit nil
func (o *StageModel) UnsetParentId() {
	o.ParentId.Unset()
}

// GetRepoId returns the RepoId field value
func (o *StageModel) GetRepoId() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.RepoId
}

// GetRepoIdOk returns a tuple with the RepoId field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetRepoIdOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.RepoId, true
}

// SetRepoId sets field value
func (o *StageModel) SetRepoId(v int64) {
	o.RepoId = v
}

// GetStarted returns the Started field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *StageModel) GetStarted() int64 {
	if o == nil || IsNil(o.Started.Get()) {
		var ret int64
		return ret
	}
	return *o.Started.Get()
}

// GetStartedOk returns a tuple with the Started field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *StageModel) GetStartedOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return o.Started.Get(), o.Started.IsSet()
}

// HasStarted returns a boolean if a field has been set.
func (o *StageModel) HasStarted() bool {
	if o != nil && o.Started.IsSet() {
		return true
	}

	return false
}

// SetStarted gets a reference to the given NullableInt64 and assigns it to the Started field.
func (o *StageModel) SetStarted(v int64) {
	o.Started.Set(&v)
}

// SetStartedNil sets the value for Started to be an explicit nil
func (o *StageModel) SetStartedNil() {
	o.Started.Set(nil)
}

// UnsetStarted ensures that no value is present for Started, not even an explicit nil
func (o *StageModel) UnsetStarted() {
	o.Started.Unset()
}

// GetStatus returns the Status field value
func (o *StageModel) GetStatus() CIStatus {
	if o == nil {
		var ret CIStatus
		return ret
	}

	return o.Status
}

// GetStatusOk returns a tuple with the Status field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetStatusOk() (*CIStatus, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Status, true
}

// SetStatus sets field value
func (o *StageModel) SetStatus(v CIStatus) {
	o.Status = v
}

// GetStopped returns the Stopped field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *StageModel) GetStopped() int64 {
	if o == nil || IsNil(o.Stopped.Get()) {
		var ret int64
		return ret
	}
	return *o.Stopped.Get()
}

// GetStoppedOk returns a tuple with the Stopped field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *StageModel) GetStoppedOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return o.Stopped.Get(), o.Stopped.IsSet()
}

// HasStopped returns a boolean if a field has been set.
func (o *StageModel) HasStopped() bool {
	if o != nil && o.Stopped.IsSet() {
		return true
	}

	return false
}

// SetStopped gets a reference to the given NullableInt64 and assigns it to the Stopped field.
func (o *StageModel) SetStopped(v int64) {
	o.Stopped.Set(&v)
}

// SetStoppedNil sets the value for Stopped to be an explicit nil
func (o *StageModel) SetStoppedNil() {
	o.Stopped.Set(nil)
}

// UnsetStopped ensures that no value is present for Stopped, not even an explicit nil
func (o *StageModel) UnsetStopped() {
	o.Stopped.Unset()
}

// GetType returns the Type field value
func (o *StageModel) GetType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *StageModel) SetType(v string) {
	o.Type = v
}

// GetUpdated returns the Updated field value
func (o *StageModel) GetUpdated() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.Updated
}

// GetUpdatedOk returns a tuple with the Updated field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetUpdatedOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Updated, true
}

// SetUpdated sets field value
func (o *StageModel) SetUpdated(v int64) {
	o.Updated = v
}

// GetVariant returns the Variant field value
func (o *StageModel) GetVariant() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Variant
}

// GetVariantOk returns a tuple with the Variant field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetVariantOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Variant, true
}

// SetVariant sets field value
func (o *StageModel) SetVariant(v string) {
	o.Variant = v
}

// GetVersion returns the Version field value
func (o *StageModel) GetVersion() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.Version
}

// GetVersionOk returns a tuple with the Version field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetVersionOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Version, true
}

// SetVersion sets field value
func (o *StageModel) SetVersion(v int64) {
	o.Version = v
}

// GetWorkflowId returns the WorkflowId field value
func (o *StageModel) GetWorkflowId() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.WorkflowId
}

// GetWorkflowIdOk returns a tuple with the WorkflowId field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetWorkflowIdOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.WorkflowId, true
}

// SetWorkflowId sets field value
func (o *StageModel) SetWorkflowId(v int64) {
	o.WorkflowId = v
}

// GetYamlProvider returns the YamlProvider field value
func (o *StageModel) GetYamlProvider() WorkflowProvider {
	if o == nil {
		var ret WorkflowProvider
		return ret
	}

	return o.YamlProvider
}

// GetYamlProviderOk returns a tuple with the YamlProvider field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetYamlProviderOk() (*WorkflowProvider, bool) {
	if o == nil {
		return nil, false
	}
	return &o.YamlProvider, true
}

// SetYamlProvider sets field value
func (o *StageModel) SetYamlProvider(v WorkflowProvider) {
	o.YamlProvider = v
}

// GetYamlResolved returns the YamlResolved field value
func (o *StageModel) GetYamlResolved() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.YamlResolved
}

// GetYamlResolvedOk returns a tuple with the YamlResolved field value
// and a boolean to check if the value has been set.
func (o *StageModel) GetYamlResolvedOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.YamlResolved, true
}

// SetYamlResolved sets field value
func (o *StageModel) SetYamlResolved(v string) {
	o.YamlResolved = v
}

// GetYamlUses returns the YamlUses field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *StageModel) GetYamlUses() string {
	if o == nil || IsNil(o.YamlUses.Get()) {
		var ret string
		return ret
	}
	return *o.YamlUses.Get()
}

// GetYamlUsesOk returns a tuple with the YamlUses field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *StageModel) GetYamlUsesOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.YamlUses.Get(), o.YamlUses.IsSet()
}

// HasYamlUses returns a boolean if a field has been set.
func (o *StageModel) HasYamlUses() bool {
	if o != nil && o.YamlUses.IsSet() {
		return true
	}

	return false
}

// SetYamlUses gets a reference to the given NullableString and assigns it to the YamlUses field.
func (o *StageModel) SetYamlUses(v string) {
	o.YamlUses.Set(&v)
}

// SetYamlUsesNil sets the value for YamlUses to be an explicit nil
func (o *StageModel) SetYamlUsesNil() {
	o.YamlUses.Set(nil)
}

// UnsetYamlUses ensures that no value is present for YamlUses, not even an explicit nil
func (o *StageModel) UnsetYamlUses() {
	o.YamlUses.Unset()
}

func (o StageModel) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o StageModel) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["action_id"] = o.ActionId
	toSerialize["arch"] = o.Arch
	toSerialize["created"] = o.Created
	toSerialize["errignore"] = o.Errignore
	toSerialize["error"] = o.Error
	toSerialize["exit_code"] = o.ExitCode
	toSerialize["id"] = o.Id
	toSerialize["is_concurrent"] = o.IsConcurrent
	toSerialize["is_matrix"] = o.IsMatrix
	toSerialize["is_resolved"] = o.IsResolved
	toSerialize["is_reusable"] = o.IsReusable
	toSerialize["job_key"] = o.JobKey
	toSerialize["kernel"] = o.Kernel
	toSerialize["kind"] = o.Kind
	toSerialize["labels"] = o.Labels
	toSerialize["limit"] = o.Limit
	toSerialize["limit_repo"] = o.LimitRepo
	toSerialize["machine"] = o.Machine
	toSerialize["name"] = o.Name
	toSerialize["needs"] = o.Needs
	toSerialize["number"] = o.Number
	toSerialize["on_failure"] = o.OnFailure
	toSerialize["on_success"] = o.OnSuccess
	toSerialize["os"] = o.Os
	if o.Outputs != nil {
		toSerialize["outputs"] = o.Outputs
	}
	toSerialize["parent_group_id"] = o.ParentGroupId
	if o.ParentId.IsSet() {
		toSerialize["parent_id"] = o.ParentId.Get()
	}
	toSerialize["repo_id"] = o.RepoId
	if o.Started.IsSet() {
		toSerialize["started"] = o.Started.Get()
	}
	toSerialize["status"] = o.Status
	if o.Stopped.IsSet() {
		toSerialize["stopped"] = o.Stopped.Get()
	}
	toSerialize["type"] = o.Type
	toSerialize["updated"] = o.Updated
	toSerialize["variant"] = o.Variant
	toSerialize["version"] = o.Version
	toSerialize["workflow_id"] = o.WorkflowId
	toSerialize["yaml_provider"] = o.YamlProvider
	toSerialize["yaml_resolved"] = o.YamlResolved
	if o.YamlUses.IsSet() {
		toSerialize["yaml_uses"] = o.YamlUses.Get()
	}
	return toSerialize, nil
}

func (o *StageModel) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"action_id",
		"arch",
		"created",
		"errignore",
		"error",
		"exit_code",
		"id",
		"is_concurrent",
		"is_matrix",
		"is_resolved",
		"is_reusable",
		"job_key",
		"kernel",
		"kind",
		"labels",
		"limit",
		"limit_repo",
		"machine",
		"name",
		"needs",
		"number",
		"on_failure",
		"on_success",
		"os",
		"parent_group_id",
		"repo_id",
		"status",
		"type",
		"updated",
		"variant",
		"version",
		"workflow_id",
		"yaml_provider",
		"yaml_resolved",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varStageModel := _StageModel{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varStageModel)

	if err != nil {
		return err
	}

	*o = StageModel(varStageModel)

	return err
}

type NullableStageModel struct {
	value *StageModel
	isSet bool
}

func (v NullableStageModel) Get() *StageModel {
	return v.value
}

func (v *NullableStageModel) Set(val *StageModel) {
	v.value = val
	v.isSet = true
}

func (v NullableStageModel) IsSet() bool {
	return v.isSet
}

func (v *NullableStageModel) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableStageModel(val *StageModel) *NullableStageModel {
	return &NullableStageModel{value: val, isSet: true}
}

func (v NullableStageModel) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableStageModel) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
